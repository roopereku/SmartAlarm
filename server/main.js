    ...
    activeNodes.forEach((n) => {
        ...
        n.instances.forEach((i) => {
            ...
            instanceData[n.ID].push({
                num: i.num, params: nodeValues[n.ID][i.num], dependencies: instanceDependencies
            })
        })
    })
...
function loadSession() {
    ...
    for (const [key, value] of Object.entries(instanceData)) {
        ...
        value.forEach((i) => {
            node.instances.push({
                num: i.num, parent: node, dependencies: []
            })
        ...
        for (let i = 0; i < n.instances.length; i++) {
            ...
            instanceData[n.ID][i].dependencies.forEach((d) => {
                ...
                activeNodes.forEach((n2) => {
                    ...
                    //	We have a match!
                    if (dep !== undefined) n.instances[i].dependencies.push(dep)
                })
            })
        }
    })
...
const server = net.createServer((client) => {
    ...
    client.on("error", (e) => {
        ...
        sendToAll({
            cmd: "dead", arg: [node.ID, node.name]
        })
    })
        ...
        jsons.forEach((json) => {
            ...
            if (msg.id !== undefined) {
                ...
                if (reconnect) {
                    ...
                    sendToAll({
                        cmd: "alive", arg: [node.ID, node.name]
                    })
                }
            }
...
function sendToNode(node, message) {
    if (node.connection !== undefined) node.connection.write(message + "\n")
}
...
function informAboutNode(node, client) {
    ...
    if (node.connection !== undefined) client.send(JSON.stringify(msg));
...
function findInstance(node, num) {
    for (let i = 0; i < node.instances.length; i++) {
        if (node.instances[i].num === num) return node.instances[i]
    }
...
function sendInstanceData(c) {
    ...
    activeNodes.forEach((n) => {
        n.instances.forEach((i) => {
            c.send(JSON.stringify({
                cmd: "passed", arg: [i.parent.ID, i.num, hasPassed(i)]
            }))
        })
    })
}
    ...
    c.on("message", (payload) => {
        try {
            ...
            if (msg.cmd === "layout") {
                ...
                if (node === undefined) msg.error = true
...
function trigger(instance) {
    ...
    sendToAll({
        cmd: "passed", arg: [instance.parent.ID, instance.num, passed]
    })
...
function findInDependencies(from, node) {
    ...
    for (let i = 0; i < from.dependencies.length; i++) {
        if (findInDependencies(from.dependencies[i], node)) return true
    }
...
function addDependency(toID, toInstance, nodeID, nodeInstance) {
    ...
    if (findInDependencies(findInstance(dep, nodeInstance), findInstance(to, toInstance))) return false
