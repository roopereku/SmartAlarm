...
editor.on('nodeCreated', function(id) {
  console.log("Node created " + id);
	let node = editor.getNodeFromId(id)
	let instance = editor.getNodesFromName(node.name).length - 1

	editor.updateNodeDataFromId(id, { instance: instance })
	addInstance(node)
})
...
function addNodeToDrawFlow(name, pos_x, pos_y) {
  ...
  switch (name) {
	...
	var facebook = `
	<div>
	  <div class="title-box"><i class="fab fa-facebook"></i> Time node test</div>
	</div>
	`;
	  editor.addNode('time:1', 1,  1, pos_x, pos_y, 'time', {}, facebook );
	  break;
	case 'slack':
	  var slackchat = `
	  <div>
