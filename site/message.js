...
ws.addEventListener('message', (event) => {
    ...
    if(msg.cmd === "getlayout") {
        ...
        if (msg.result === false) cutDependency(msg.arg[1], msg.arg[3])
    } else if (msg.cmd === "passed") {
        console.log(msg.arg)
        highlightNodeByID(msg.arg[0], msg.arg[1], msg.arg[2] ? "green" : "red")
    } else if (msg.cmd === "dead") {
        disableNode(msg.arg[0])
        Swal.fire('Connection dead', "Node " + msg.arg[1] + " has lost it's connection", 'error')
...
function sendLayout(layoutJSON) {
    ...
    sendMessage({
        cmd: "layout", arg: [layoutJSON]
    })
}

function setParameters(node, params) {
    sendMessage({
        cmd: "parameters", arg: [node.name, node.data.instance, params]
    })
}

function addDependency(to, node) {
    sendMessage({
        cmd: "depend", arg: [to.name, to.data.instance, node.name, node.data.instance]
    })
}

function removeDependency(from, node) {
    sendMessage({
        cmd: "undepend", arg: [from.name, from.data.instance, node.name, node.data.instance]
    })
}
...
function addInstance(node) {
    ...
    sendMessage({
        cmd: "instance", arg: [node.name, node.data.instance]
    })
}
